  ...
  async init(data) {
    try {
      ...
      transport.on('connect', ({ dtlsParameters }, callback) => {
        this.ws.send({
          handler: 'audioroom', action: 'connect-transport',
          roomId: data.roomId, direction: 'send', dtlsParameters
        });
        callback();
      });
      ...
      transport.on('produce', ({ kind, rtpParameters }, callback) => {
        this.ws.send({
          handler: 'audioroom', action: 'produce',
          roomId: data.roomId, kind, rtpParameters, requestId: "bot-mic-" + Date.now()
        });
      ...
      console.log(`[SUCCESS] Mic is Active in ${data.roomId}`);
    } catch (e) { console.error("Mediasoup Error:", e); }
  }
}
